...
async function signup(email: string) {
  ...
  await app.request('/api/auth/me', { headers: auth }, env); // creates the user row
  return auth;
}
...
beforeEach(async () => {
  ...
  const auth = await signup('p1@b.com');
...
describe('photos', () => {
  it('uploads a photo and lists it back with a fetchable url', async () => {
    const auth = await signup('p2@b.com');
    const cellarId = (await (await app.request('/api/cellars', { headers: auth }, env)).json<any[]>())[0].id;
    ...
    const uploadRes = await app.request(`/api/bottles/${bottleId}/photos`, { method: 'POST', body: form, headers: auth }, env);
    expect(uploadRes.status).toBe(200);
    const uploaded = await uploadRes.json<{ id: number; url: string }>();

    const listRes = await app.request(`/api/bottles/${bottleId}/photos`, { headers: auth }, env);
    const list = await listRes.json<any[]>();
    expect(list).toHaveLength(1);

    const fileRes = await app.request(uploaded.url, { headers: auth }, env);
  ...
  it('rejects an upload whose bytes are not a real image, even with a spoofed content-type', async () => {
    const auth = await signup('p3@b.com');
  ...
  it('requires a session to fetch a photo by key', async () => {
    const auth = await signup('p4@b.com');
  ...
  it('hides a photo from a caller who is not a member of the bottle cellar', async () => {
    ...
    const form = new FormData();
    form.append('file', new Blob([JPEG_MAGIC], { type: 'image/jpeg' }), 'label.jpg');
    const uploadRes = await app.request(`/api/bottles/${bottleId}/photos`, { method: 'POST', body: form, headers: memberAuth }, env);
    const uploaded = await uploadRes.json<{ id: number; url: string }>();

    const outsiderAuth = await signup('outsider@b.com');
    const fileRes = await app.request(uploaded.url, { headers: outsiderAuth }, env);
